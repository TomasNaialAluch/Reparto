...
export default function VentaDetalle({ venta, cliente, onEdit }) {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#212529' }}>{venta.numero}</div>
            <div style={{ fontSize: '0.75rem', color: '#6c757d' }}>{venta.tipo} · {new Date(venta.fecha).toLocaleDateString('es-AR')}</div>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {venta.items.map((item, i) => (
              <tr key={i}>
                <td style={{ ...td, fontWeight: 600 }}>{item.producto}</td>
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 700, fontSize: '1.05rem', color: '#212529' }}>{formatCurrency(venta.total)}</span>
      </div>

      {showPrint && (
        <PrintDocument
          data={{ ...venta, clienteNombre: cliente?.razonSocial }}
          type="venta"
